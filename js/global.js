...
jQuery(function($) {
	...
	//SVG Map accordion
	$(document).on('click', '.checkboxEntry span', function(event) {
		$(this).closest('.checkboxEntry').toggleClass('open').next('.toggleCity').slideToggle(300);
		return false;
	});

	$(document).on('click', '.checkboxEntry input', function(event) {
		$(this).closest('.checkboxEntry').toggleClass('active');
		var cat = $(this).closest('.checkboxEntry').attr('data-catid');
		$('.mapplic-tooltip-close').click();
		if($(this).closest('.checkboxEntry').hasClass('active')){
			$('.mapplic-pin.'+cat).fadeIn();
		} else {
			$('.mapplic-pin.'+cat).fadeOut();
		}
		return false;
	});
	...
	$('.clearCheckboxs').on('click', function() {
		$('.checkboxEntry input').prop('checked', true);
		$('.checkboxEntry').addClass('active').removeClass('open').next('.toggleCity').slideUp(300);
		$('.mapplic-pin').fadeIn();
		$('.mapplic-tooltip-close').click();
	});

	//create sidebar
	_functions.caraMapSidebar = function(element) {
		if(element.categories){
			$.each(element.categories, function(index, category) {
				$('.brandsCheckBoxWrap').append(
					'<div class="checkboxEntry brandsCheck light mapplic-list-shown active" data-catid="'+category.id+'">'+
                        '<input type="checkbox" /><span class="sr category-sr mapplic-list-shown">'+category.title+'</span>'+
                        '<i>0</i>'+
                    '</div>'+
                    '<ul class="toggleCity showAll"></ul>');
			});
		}

		if(element.levels[0].locations){
			$.each(element.levels[0].locations, function(index, location) {
				var cat = $('.checkboxEntry[data-catid="'+location.category+'"]');
				var prev_count = parseInt(cat.find('i').text());
				cat.next('.toggleCity').append('<li class="sr list-sr mapplic-list-shown" data-id="'+location.id+'">'+location.city+'<span class="sr hide-sr">'+location.category+'</span></li>');
				cat.find('i').text(prev_count+1);
			});
		}
	}

	//Search
	$('.searchWrapper input').keyup(function(event) {
		var keyword = $(this).val();
		$('.sr').each(function() {
			if ($(this).text().search(new RegExp(keyword, "i")) < 0) {
				if($(this).hasClass('list-sr')){
					$(this).removeClass('mapplic-list-shown').hide();
				}
			} else {
				if($(this).hasClass('list-sr')){
					$(this).addClass('mapplic-list-shown').show();
				} 
				if($(this).hasClass('category-sr')){
					console.log($(this).closest('.checkboxEntry').next('.toggleCity').find('li').text());
					setTimeout(function(){
					$(this).closest('.checkboxEntry').next('.toggleCity').find('li').addClass('mapplic-list-shown testclass').show();
					}, 10);
				}
			}
		});
		...
		setTimeout(function(){
			$('.toggleCity').each(function() {
				var count = $(this).find('li.mapplic-list-shown').length;
				if(count === 0){
					$(this).closest('ul').prev('.checkboxEntry').fadeOut(300);
				} else {
					$(this).closest('ul').prev('.checkboxEntry').slideDown(300);
				}
				$(this).closest('ul').prev('.checkboxEntry').find('i').text(count);
			});
		},10);
	});

	//city click
	$(document).on('click', '.toggleCity li', function(event) {
		var id = $(this).attr('data-id');
		$('.mapplic-pin[data-location="'+id+'"]').click();
	});
